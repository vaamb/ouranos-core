...
def test_current_data(client_admin: TestClient):
    ...
    assert value[1] == g_data.system_data_dict["system_uid"]
    assert value[2] == g_data.system_data_dict["CPU_used"]
    assert value[3] == g_data.system_data_dict["CPU_temp"]
    assert value[4] == g_data.system_data_dict["RAM_process"]
    assert value[5] == g_data.system_data_dict["RAM_used"]
    assert value[6] == g_data.system_data_dict["DISK_used"]
