...
from ouranos.core.database.models.caching import (
    CachedCRUDMixin, cached, create_hashable_key, sessionless_hasher)
...
class Ecosystem(Base, CachedCRUDMixin, InConfigMixin):
    ...
    @classmethod
    @cached(cache_ecosystems_has_recent_data, key_hasher=sessionless_hasher)
    ...
    @classmethod
    @cached(cache_ecosystems_has_active_actuator, key_hasher=sessionless_hasher)
    ...
    @cached(cache_sensors_data_skeleton, key_hasher=sessionless_hasher)
...
class SensorDataRecord(BaseSensorDataRecord, ArchivableMixin):
    ...
    @classmethod
    @cached(cache_sensors_value, key_hasher=sessionless_hasher)
    async def get_timed_values(
            cls,
            session: AsyncSession,
            *,
...
class GaiaWarning(Base):
    ...
    @classmethod
    @cached(cache_warnings, key_hasher=sessionless_hasher)
    async def get_multiple(
            cls,
            session: AsyncSession,
            /,
...
class CameraPicture(Base, CRUDMixin):
    ...
    path: Mapped[str] = mapped_column(sa.String(length=512))
    dimension: Mapped[tuple] = mapped_column(sa.JSON)
    depth: Mapped[str] = mapped_column(sa.String(length=16))
    timestamp: Mapped[datetime] = mapped_column(UtcDateTime)
