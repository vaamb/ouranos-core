...
from src.dataspace import WEATHER_MEASURES, weatherData
...
class Weather(serviceTemplate):
    ...
    def update_weather_data(self) -> None:
        if is_connected():
            self._logger.debug("Trying to update weather data")
            try:
                ...
                self._logger.error("ConnectionError: cannot update weather data")
                with self.mutex:
                    self._data.clear()
            else:
                raw_data = {
            ...
            self._logger.error("ConnectionError: could not update weather data")
            with self.mutex:
                self._data.clear()
